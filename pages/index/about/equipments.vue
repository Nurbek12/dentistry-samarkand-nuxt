    ...
    <div class="w-full">
        <div class="container px-2 md:px-8 py-16">
            <div class="text-center pt-20 pb-10">
                <h1 class="font-medium text-teal-600 text-3xl">Технологии и оборудование</h1>
                <p class="text-base text-gray-600">
                    Наши Стоматологические Услуги - Заботимся о Здоровье Вашей Улыбки
                </p>
            </div>
            <div class="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                <card-equipment v-for="n,i in items" :key="i" :data="n" />
            </div>
        </div>
    </div>
</template>
...
const items = ref([])
const init = async () => {
    const data = await $fetch('/api/equipment/clients', {
